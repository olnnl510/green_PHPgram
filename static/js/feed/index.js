(function () {
    ...
    const feedObj = {
        ...
        getFeedList: function () { // 백엔드와 통신, json형태로 feed 테이블 select한것을 받아옴
            this.showLoading();
            const param = {
        ...
        makeFeedList: function (list) { // 화면에 item 막 집어넣는 컨트롤타워. 반복문 돌림
            if (list.length !== 0) {
                list.forEach(item => {
                    const divItem = this.makeFeedItem(item);
                    this.containerElem.appendChild(divItem); // return 받은것을 이곳에 추가
                });
            }
        ...
        makeFeedItem: function (item) { // 부하직원. item 하나하나 만드는 친구. 정보주면 하나하나 return
            console.log(item);
            ...
            heartIcon.classList.add(item.isFav === 1 ? 'fas' : 'far'); // 채워진 : 안채워진 하트가 생김
